...
import App, { acceptanceThresholdPassed, resolveTargetHost } from './App';
...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith('/api/ai/probe')) return jsonResponse({ connected: true, model: 'gpt-5.6-terra', protocol: 'responses', elapsedMs: 123, verifiedModelId: 'gpt-5.6-terra', capabilities: { schema: 'passed', multiTurn: 'passed', vision: 'passed' }, probeVersion: 'agent-first-v1' });
...
describe('App', () => {
  ...
  it('creates a fixed 65 by 5 blocked acceptance batch without claiming verification', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /电商验收/ }));
    expect(await screen.findByText('325 次验收控制台')).toBeInTheDocument();
  ...
  it('starts Agent-first exploration from the natural-language goal without a fixed plan', async () => {
    ...
    expect(payload).toMatchObject({ targetUrl: plan.base_url, enableVisualFallback: true });
