...
_NEGATIVE_CONSTRAINT = re.compile(
    r"^(?:请)?(?:不要|不得|禁止|严禁|避免|不允许|不可|不能|不应|不再|不)(?:\s|登录|点击|填写|输入|选择|提交|发送|支付|领取|修改|删除|收藏|关注|加入|操作|访问|下载|上传)"
)
...
def plan_from_draft(
    *,
    ...
    for clause in clauses:
        ...
        for piece in pieces:
            normalized_piece = re.sub(r"^(然后|随后|接着|并且|并|再|最后)", "", piece).strip()
            if _NEGATIVE_CONSTRAINT.match(normalized_piece):
                continue
...
def _parse_piece(text: str) -> Step | Assertion | None:
    ...
    if _NEGATIVE_CONSTRAINT.match(normalized):
        return None
