...
from gui_agent.acceptance import AcceptanceBatchManager, CompiledScenario, dry_run_executor
    ...
    return [
        CompiledScenario(
            scenario=BenchmarkScenario.model_validate({
                ...
                "prerequisites": [{"description": "测试环境可用"}],
                "goal": f"完成通用业务目标 {index}",
                "steps": [{"id": "step-1", "businessAction": "检查页面"}],
                "assertions": [{"id": "assertion-1", "businessFact": "页面可见"}],
                "dangerousPolicy": {"mode": "confirm"}, "cleanup": {"required": False},
                "evidenceRequirements": ["screenshot"], "bindingStatus": "ready",
                "executablePlan": {"name": f"S{index:02d}"},
            }),
            plan=PlanModel(name=f"通用场景 {index}", base_url="https://example.com", steps=[Step(action="screenshot")]),
            account_id="tester",
            file_ids=(),
        )
