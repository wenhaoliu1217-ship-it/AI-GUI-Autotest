...
$browserCleanupLog = Join-Path $packageRoot 'browser-cleanup.log'
$pidFile = Join-Path $packageRoot 'server.pid'
$runnerImage = 'ai-gui-runner:1.32.00'
$runnerArchives = @(
  (Join-Path $dockerArchiveRoot 'ai-gui-runner-1.32.00.tar'),
  (Join-Path $dockerArchiveRoot 'ai-gui-runner-1.32.00.tar.gz'),
  (Join-Path $dockerArchiveRoot 'ai-gui-runner-1.32.00.tgz')
)
...
$managedBrowserPath = $edgeCandidates | Where-Object { $_ -and (Test-Path -LiteralPath $_ -PathType Leaf) } | Select-Object -First 1
$managedBrowserName = 'Microsoft Edge'
if (-not $managedBrowserPath) {
...
if (Test-Path -LiteralPath $browserSessions -PathType Container) {
  try {
    $staleSessionFiles = @(Get-ChildItem -LiteralPath $browserSessions -File -ErrorAction Stop)
    foreach ($sessionFile in $staleSessionFiles) {
      Remove-Item -LiteralPath $sessionFile.FullName -Force -ErrorAction Stop
    }
    if ($staleSessionFiles.Count -gt 0) {
      "$(Get-Date -Format o) Removed $($staleSessionFiles.Count) stale browser session file(s)." |
        Add-Content -LiteralPath $browserCleanupLog -Encoding UTF8
    }
  }
  catch {
    $cleanupMessage = "$(Get-Date -Format o) Could not clear stale browser sessions: $($_.Exception.Message)"
    $cleanupMessage | Add-Content -LiteralPath $browserCleanupLog -Encoding UTF8
...
if ($cdpPort -gt 9232) {
  throw 'No available local browser-control port was found between 9222 and 9232.'
}
$env:GUI_BROWSER_CDP_URL = "http://127.0.0.1:$cdpPort"
$env:GUI_BROWSER_NAME = $managedBrowserName

$requestedRunnerMode = if ($env:GUI_RUNNER_MODE -eq 'process') { 'process' } else { 'container' }
if ($requestedRunnerMode -eq 'container') {
...
if (-not (Test-DockerReady -DockerPath $docker)) {
  ...
  if (-not $dockerReady) {
    throw 'Docker Engine did not become ready within 3 minutes. Open Docker Desktop, resolve its startup error, and run 双击启动AI测试.bat again.'
  }
}
...
$env:GUI_RUNNER_IMAGE = $runnerImage
  ...
  Write-Host '[3/5] Using the explicitly requested local Runner process mode...'
  Write-Host '[4/5] Skipping the isolated Runner image check for this session...'
}
$env:GUI_RUNNER_MODE = $requestedRunnerMode
...
try {
  ...
  if ($env:GUI_SKIP_BROWSER -ne '1') {
    ...
    do {
      Start-Sleep -Milliseconds 250
      try {
        $browserResponse = Invoke-WebRequest -Uri ($env:GUI_BROWSER_CDP_URL + '/json/version') -UseBasicParsing -TimeoutSec 2
        if ($browserResponse.StatusCode -eq 200) { $browserReady = $true }
      }
      catch { }
    } while (-not $browserReady -and (Get-Date) -lt $browserDeadline)
    if (-not $browserReady) {
      throw "$managedBrowserName did not expose the shared GUI browser connection."
    }
    ...
    do {
      Start-Sleep -Milliseconds 250
      try {
        $browserTargets = @(Invoke-RestMethod -Uri ($env:GUI_BROWSER_CDP_URL + '/json/list') -TimeoutSec 3)
        if (-not $guiTargetId) {
          $guiTarget = $browserTargets |
            Where-Object {
              [string]$_.type -eq 'page' -and
              ([string]$_.url).StartsWith($url, [System.StringComparison]::OrdinalIgnoreCase)
            } |
            Select-Object -First 1
          if ($guiTarget) { $guiTargetId = [string]$guiTarget.id }
        }
        if ($guiTargetId) {
          foreach ($target in $browserTargets) {
            if ([string]$target.type -ne 'page' -or [string]$target.id -eq $guiTargetId) { continue }
            Invoke-WebRequest `
              -Uri ($env:GUI_BROWSER_CDP_URL + '/json/close/' + [string]$target.id) `
              -UseBasicParsing `
              -TimeoutSec 3 | Out-Null
            "$(Get-Date -Format o) Closed one restored browser page." |
              Add-Content -LiteralPath $browserCleanupLog -Encoding UTF8
          }
        }
      }
      catch {
        $cleanupMessage = "$(Get-Date -Format o) Browser page cleanup attempt failed: $($_.Exception.Message)"
        $cleanupMessage | Add-Content -LiteralPath $browserCleanupLog -Encoding UTF8
      }
  ...
  Write-Host "GUI and login pages share one $managedBrowserName window."
  Write-Host 'Screenshots and reports are stored in the artifacts folder.'
  if ($env:GUI_AUTO_STOP -ne '1') {
    Write-Host 'Run the close BAT file when you want to close the GUI.'
    while ($server -and -not $server.HasExited) {
