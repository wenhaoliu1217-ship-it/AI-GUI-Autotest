...
def _cesium_upload_form_wait_decision(scenario: AgentScenario, observation: Observation, history: list[StepResult]) -> AgentDecision | None:
    goal = scenario.goal.lower()
    if not is_cesium_target(observation.url) or "/addasset" not in observation.url or not any(marker in goal for marker in ("上传", "upload")):
        return None
    ...
    if 'button "cancel"' not in facts or "add files" not in facts:
        return None
    return AgentDecision(kind="action", action=Step(action="screenshot", description=_UPLOAD_FORM_WAIT_DESCRIPTION, waitBeforeMs=5_000, effect_kind="browse_search_filter_sort", effect_level="read_only"), reason="上传入口已显示文件入口、云来源选项和 Cancel，先等待表单稳定再安全返回。", progress_assessment="progress")


def _cesium_upload_form_cancel_decision(scenario: AgentScenario, observation: Observation, history: list[StepResult]) -> AgentDecision | None:
    goal = scenario.goal.lower()
    if not is_cesium_target(observation.url) or "/addasset" not in observation.url or not any(marker in goal for marker in ("取消", "返回", "cancel", "back")):
        return None
    if not any(step.target_summary.startswith(_UPLOAD_FORM_WAIT_DESCRIPTION) for step in history) or any(step.target_summary.startswith(_UPLOAD_FORM_CANCEL_DESCRIPTION) for step in history):
        return None
    if 'button "cancel"' not in observation.accessibility_summary.lower():
        return None
    return AgentDecision(kind="action", action=Step(action="click", locator=Locator(role="button", name="Cancel", exact=True), description=_UPLOAD_FORM_CANCEL_DESCRIPTION, effect_kind="browse_search_filter_sort", effect_level="read_only"), reason="表单入口和来源选项已经取证，点击 Cancel 可在不选择或提交文件的前提下返回。", progress_assessment="progress")


def _cesium_upload_form_complete_decision(scenario: AgentScenario, observation: Observation, history: list[StepResult]) -> AgentDecision | None:
    goal = scenario.goal.lower()
    ...
    if "/assets" not in observation.url or "my assets" not in observation.title.lower():
        return None
    return AgentDecision(kind="complete", reason="上传资产入口已打开并稳定显示 Add files、S3、Azure、Sketchfab 和 Cancel；未选择文件时没有可提交的资产必填内容，已通过 Cancel 返回 My Assets，未创建资产。", progress_assessment="progress")
