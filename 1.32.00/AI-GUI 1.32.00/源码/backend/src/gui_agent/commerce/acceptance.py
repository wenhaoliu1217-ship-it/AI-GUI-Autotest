...
class AcceptanceBatchStore:
        ...
        for scenario in scenarios:
            ...
            for repeat in range(1, self.repeats + 1):
                attempts.append({
                    ...
                    "clarificationRequired": False, "clarificationRecovered": None,
                    "zeroToleranceIncidents": {},
                })
        batch = {
            "id": batch_id, "profile": "agent-first-commerce-1.32.00", "status": "blocked" if all(a["status"] == "blocked" for a in attempts) else "ready",
    ...
    def record_attempt(self, batch_id: str, attempt_id: str, result: dict) -> dict:
        ...
        classification = result.get("resultClassification", "unverified")
        if classification not in {"agent_passed", "agent_failed", "fixed_passed", "fixed_failed", "fallback_passed", "unverified"}:
            raise AcceptanceBatchError("执行结果分类非法")
        if status == "passed" and classification not in {"agent_passed", "fixed_passed", "fallback_passed"}:
            raise AcceptanceBatchError("通过结果必须声明真实执行分类")
        clarification_required = bool(result.get("clarificationRequired"))
        clarification_recovered = result.get("clarificationRecovered")
        if clarification_required and clarification_recovered is not True:
            status = "failed"
        attempt.update({
            "status": status,
            ...
            "clarificationRecovered": clarification_recovered,
    ...
    def _summarize(self, batch: dict) -> dict:
        ...
        p0_agent_passed = sum(a["status"] == "passed" and a.get("resultClassification") == "agent_passed" for a in p0)
        agent_passed = sum(a["status"] == "passed" and a.get("resultClassification") == "agent_passed" for a in attempts)
        clarifications = [a for a in attempts if a.get("clarificationRequired")]
        thresholds = {
            "p0AgentPassRate": {"actual": p0_agent_passed / len(p0) if p0 else 0, "required": 1.0},
            "overallAgentPassRate": {"actual": agent_passed / len(attempts) if attempts else 0, "required": 0.95},
            "clarificationRecoveryRate": {"actual": sum(a.get("clarificationRecovered") is True for a in clarifications) / len(clarifications) if clarifications else 1.0, "required": 1.0},
