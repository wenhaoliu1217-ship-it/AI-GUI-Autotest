...
def evaluate_commerce_action(request: CommerceActionRequest) -> CommercePolicyDecision:
    if request.action == CommerceAction.PAY:
        return CommercePolicyDecision(
            allowed=False,
            riskLevel=RiskLevel.HIGH_RISK_WRITE,
            requiresConfirmation=True,
            reason="真实支付是绝对禁止动作；测试最高只能到支付前并选择付款方式",
        )
    ...
    if request.environment == CommerceEnvironment.PRODUCTION_READONLY:
        if request.action == CommerceAction.SUBMIT_ORDER:
            missing = _production_order_controls(request)
            return CommercePolicyDecision(
                allowed=not missing,
                riskLevel=RiskLevel.HIGH_RISK_WRITE,
                requiresConfirmation=True,
                reason=("正式站受控提交未支付订单" if not missing else "正式站提交订单缺少完整专项授权"),
                missingControls=missing,
            )
...
def _production_order_controls(request: CommerceActionRequest) -> list[str]:
    missing = _reversible_controls(request, require_explicit_production_authorization=True)
    if not request.fixed_product_ref:
        missing.append("固定测试商品引用")
    if not request.fixed_address_ref:
        missing.append("固定测试地址密钥引用")
    if not request.written_authorization_ref:
        missing.append("正式站提交订单书面授权编号")
    if not request.automatic_cancellation_verified:
        missing.append("自动取消与零残留验证")
    return missing
