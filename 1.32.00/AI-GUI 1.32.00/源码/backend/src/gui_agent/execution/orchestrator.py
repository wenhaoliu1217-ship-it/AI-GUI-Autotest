...
class RunOrchestrator:
        ...
        try:
            while True:
                ...
                if message is not None:
                    ...
                    if kind == "progress":
                        ...
                        action_deadline = (
                            monotonic() + self._action_watchdog_seconds(config)
                            if active_step else None
                        )
                    elif kind == "confirmation_requested":
                        action_deadline = None
                        paused_at = paused_at or monotonic()
                        ...
                        action_deadline = (
                            monotonic() + self._action_watchdog_seconds(config)
                            if active_step and message["payload"].get("decision") == "approved"
                            else None
                        )
                        ...
                        break
                now = monotonic()
                if action_deadline is not None and now >= action_deadline and forced_reason is None:
                    forced_reason = "action_timeout_exceeded"
                    forced_at = now
                    try:
                ...
                if forced_reason and forced_at is not None and now >= forced_at + config.isolation_cancel_grace_seconds:
                    break
            ...
            if forced_reason:
                ...
                self._write_container_failure(
                    ...
                    failed=forced_reason == "action_timeout_exceeded",
                )
                final_received = True
            elif final_received:
                handle.wait(3)
    ...
    def _write_container_failure(
        ...
        *,
        ...
        state = self.read(run_id, config.artifacts_root) or {}
        state.update({
            "status": Status.CANCELLED.value if cancelled else Status.ERROR.value if failed else Status.SYSTEM_ERROR.value,
            "ended_at": datetime.now().astimezone().isoformat(),
    ...
    @staticmethod
    def _action_watchdog_seconds(config: RunnerConfig) -> float:
        if config.action_watchdog_seconds is not None:
            return max(0.05, config.action_watchdog_seconds)
        return max(60.0, config.timeout_ms / 1000 * 2 + 15.0)
