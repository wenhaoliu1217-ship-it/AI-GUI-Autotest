...
def test_health_declares_real_engine() -> None:
    ...
    assert response.json()["appVersion"] == "1.32.00"


def test_cesium_acceptance_api_exposes_all_cases_without_false_passes() -> None:
    response = client.get("/api/benchmarks/cesium-ion")

    assert response.status_code == 200
    payload = response.json()
    assert payload["summary"]["total"] == 60
    assert payload["summary"]["passed"] == 0
    assert payload["cases"][0]["id"] == "C01" and payload["cases"][-1]["id"] == "C60"
    assert payload["testData"]["manifestStatus"] == "blocked"


def test_cesium_run_rejects_unclassified_side_effects_before_browser_launch() -> None:
    response = client.post("/api/runs", json={
        "plan": {
            "name": "C05", "base_url": "https://ion.cesium.com",
            "steps": [{"action": "navigate", "target": "/assets"}], "assertions": [],
        },
        "asyncExecution": False,
    })

    assert response.status_code == 422
    assert response.json()["detail"] == "Cesium 计划第 1 步缺少 effect_kind/effect_level"
...
def test_commerce_policy_endpoint_blocks_production_order_submission() -> None:
    ...
    assert "缺少完整专项授权" in response.json()["reason"]
...
def test_commerce_assurance_endpoints_reject_production_and_duplicate_side_effects() -> None:
    inventory = client.post("/api/commerce/assurance/inventory-race/evaluate", json={
        "environment": "production_readonly",
        "evidence": {
            "stockBefore": 1, "stockAfter": 0,
            "attempts": [
                {"session": "A", "outcome": "reserved", "requestSha256": "a" * 64, "startedAt": "t1", "endedAt": "t2"},
                {"session": "B", "outcome": "rejected", "requestSha256": "b" * 64, "startedAt": "t1", "endedAt": "t2"},
            ],
        },
    })
    assert inventory.status_code == 422

    callbacks = client.post("/api/commerce/assurance/callback-idempotency/evaluate", json={
        "environment": "isolated_transaction",
        "observations": [
            {"kind": "refund", "idempotencyKeySha256": "a" * 64, "payloadSha256": "b" * 64, "result": "applied", "observedAt": "t1"},
            {"kind": "refund", "idempotencyKeySha256": "a" * 64, "payloadSha256": "b" * 64, "result": "applied", "observedAt": "t2"},
        ],
    })
    assert callbacks.status_code == 422
    assert "重复记账" in callbacks.json()["detail"]
...
def test_local_plan_generation_rejects_project_forbidden_action() -> None:
    project = client.post(
        "/api/projects",
        json={"name": "只读项目", "baseUrl": "https://example.com", "forbiddenActions": ["提交订单"]},
    ).json()
    ...
    assert response.status_code == 422
    assert "命中禁止动作：提交订单" in response.json()["detail"]
...
def test_cesium_project_includes_required_official_api_host(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(server, "PROJECT_STORE", ProjectStore(tmp_path / "projects"))

    response = client.post(
        "/api/projects",
        json={"name": "Cesium ion", "baseUrl": "https://ion.cesium.com"},
    )

    assert response.status_code == 200, response.text
    assert response.json()["allowedHosts"] == ["ion.cesium.com", "api.cesium.com"]
...
def test_generic_website_resolution_endpoint_returns_the_observed_destination(monkeypatch) -> None:
    monkeypatch.setattr(server, "resolve_public_url", lambda url: {
        "url": "https://destination.example/home",
        "changed": True,
        "redirectChain": [url, "https://destination.example/home"],
    })

    response = client.post("/api/websites/resolve", json={"url": "https://short.example/go?utm_source=search"})

    assert response.status_code == 200
    assert response.json()["url"] == "https://destination.example/home"
    assert response.json()["redirectChain"][0].startswith("https://short.example/")
    ...
    response = client.post(
        ...
        json={
            ...
            "modelDataAuthorization": {
                "siteHost": "example.com", "allowDom": True, "allowScreenshots": False,
            },
            "projectId": project["id"],
        },
    )
...
def test_planless_cesium_agent_run_uses_read_only_navigation_policy(monkeypatch) -> None:
    class FakeOrchestrator:
        def start(self, saved_plan, config):
            step = saved_plan.steps[0]
            assert step.effect_kind == "browse_search_filter_sort"
            assert step.effect_level.value == "read_only"
            assert config.cesium_policy_enabled is True
            now = "2026-07-25T00:00:00+08:00"
            return {
                "run_id": "cesium-agent-run-test", "plan_name": saved_plan.name, "role": None,
                "base_url_summary": saved_plan.base_url, "status": "queued",
                "started_at": now, "ended_at": now, "steps": [], "assertions": [],
                "reproduction_steps": [], "cause_hints": [], "findings": [],
                "completion_reason": "queued", "model_calls": 0,
    ...
    response = client.post(
        "/api/agent-runs",
        json={
            "targetUrl": "https://ion.cesium.com",
            "scenario": {
                "name": "Cesium 登录状态检查",
                "goal": "检查当前保存的登录状态是否有效",
                "forbiddenActions": ["退出登录"],
            },
            "settings": {
                "protocol": "chat_completions", "baseUrl": "https://api.example.com",
                "model": "test-model", "apiKey": "memory-only-test-key",
            },
            "modelDataAuthorization": {
                "siteHost": "ion.cesium.com", "allowDom": True, "allowScreenshots": False,
            },
        },
    )
    ...
    assert response.status_code == 200, response.text
    assert response.json()["status"] == "queued"
    assert "memory-only-test-key" not in response.text
