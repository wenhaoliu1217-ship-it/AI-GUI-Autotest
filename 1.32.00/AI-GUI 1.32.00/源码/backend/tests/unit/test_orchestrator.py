...
def _stubborn_action_runner(plan: ExecutionPlan, config: RunnerConfig):
    started = datetime.now().astimezone()
...
def _clarification_pause_runner(plan: ExecutionPlan, config: RunnerConfig):
    started = datetime.now().astimezone()
    assert config.clarification_callback is not None
    answer = config.clarification_callback("retry model", 0)
    if not answer:
        raise RuntimeError("clarification was not answered")
    time.sleep(0.05)
    return RunResult(
        run_id=config.run_id or "missing",
        plan_name=plan.name,
...
def test_orchestrator_answers_clarification_and_resumes_the_same_run(tmp_path: Path) -> None:
    resumed = Event()

    def fake_runner(plan: ExecutionPlan, config: RunnerConfig):
        started = datetime.now().astimezone()
        assert config.clarification_callback is not None
        answer = config.clarification_callback("请选择测试租户", 1)
        assert answer == "QA 租户"
        config.clarification_history.append({
            "round": 1, "question": "请选择测试租户", "answer": answer,
        })
        resumed.set()
        return RunResult(
            run_id=config.run_id or "missing",
            ...
            ended_at=datetime.now().astimezone(),
            completion_reason="agent_goal_completed",
            clarification_history=list(config.clarification_history),
        ), tmp_path / str(config.run_id)
    ...
    while state["status"] != "waiting_for_clarification" and time.time() < deadline:
        time.sleep(0.01)
        state = orchestrator.read(run_id, tmp_path)

    assert state["pending_clarification"]["question"] == "请选择测试租户"
    answered = orchestrator.answer_clarification(
        run_id, tmp_path, state["pending_clarification"]["id"], "QA 租户", "tester"
    )
    assert answered["run_id"] == run_id
    assert resumed.wait(1)
    deadline = time.time() + 2
    final = orchestrator.read(run_id, tmp_path)
    while final["status"] != "passed" and time.time() < deadline:
        time.sleep(0.01)
        final = orchestrator.read(run_id, tmp_path)
    assert final["run_id"] == run_id
    assert final["clarification_history"][0]["answer"] == "QA 租户"
...
def test_isolated_orchestrator_pauses_wall_clock_while_waiting_for_user(tmp_path: Path) -> None:
    orchestrator = RunOrchestrator(runner=_clarification_pause_runner, isolated=True)
    queued = orchestrator.start(
        _plan(),
        RunnerConfig(
            artifacts_root=tmp_path,
    ...
    state = orchestrator.read(run_id, tmp_path)
    assert state["status"] == "waiting_for_clarification"
    orchestrator.answer_clarification(
        run_id, tmp_path, state["pending_clarification"]["id"], "retry", "tester"
    )

    deadline = time.time() + 2
    final = orchestrator.read(run_id, tmp_path)
    while final["status"] in {"running", "waiting_for_clarification"} and time.time() < deadline:
        time.sleep(0.01)
        final = orchestrator.read(run_id, tmp_path)
    assert final["status"] == "passed"


def test_isolated_orchestrator_enforces_per_action_watchdog(tmp_path: Path) -> None:
    orchestrator = RunOrchestrator(runner=_stubborn_action_runner, isolated=True)
    ...
    final = orchestrator.run_blocking(
        ...
        RunnerConfig(
            artifacts_root=tmp_path,
            max_duration_seconds=10,
            action_watchdog_seconds=0.1,
            isolation_cancel_grace_seconds=0.1,
        ),
    )

    assert final["status"] == "error"
    assert final["completion_reason"] == "action_timeout_exceeded"
    assert final["goal_status"] == "incomplete"
    assert "打开受保护页面" in final["system_error"]
    assert final["runner_isolation"]["forced_termination"] is True
