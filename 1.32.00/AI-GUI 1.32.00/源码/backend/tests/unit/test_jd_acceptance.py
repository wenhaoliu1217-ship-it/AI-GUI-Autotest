...
def test_batch_cancel_resume_retry_and_attempt_evidence(tmp_path) -> None:
    ...
    recorded = store.record_attempt(batch["id"], "J01#1", {
        ...
        "resultClassification": "agent_failed",
...
def test_acceptance_api_and_report_use_persistent_store(tmp_path, monkeypatch) -> None:
    ...
    assert all(name in report.text for name in (
        "p0AgentPassRate", "overallAgentPassRate", "clarificationRecoveryRate", "stableReplayRate",
...
def test_fixed_success_cannot_satisfy_agent_first_gates(tmp_path) -> None:
    store = AcceptanceBatchStore(tmp_path / "batches", _catalog(tmp_path, blocked=False))
    batch = store.start()
    recorded = store.record_attempt(batch["id"], "J01#1", {
        "status": "passed", "runId": "fixed-1", "resultClassification": "fixed_passed",
        "evidenceCompleteness": 1.0, "stableReplay": True,
        "amountAccurate": True, "cleanupComplete": True,
    })
    assert recorded["summary"]["thresholds"]["p0AgentPassRate"]["actual"] == 0
    assert recorded["summary"]["thresholds"]["overallAgentPassRate"]["actual"] == 0


def test_failed_clarification_recovery_forces_attempt_failure(tmp_path) -> None:
    store = AcceptanceBatchStore(tmp_path / "batches", _catalog(tmp_path, blocked=False))
    batch = store.start()
    recorded = store.record_attempt(batch["id"], "J01#1", {
        "status": "passed", "runId": "agent-1", "resultClassification": "agent_passed",
        "clarificationRequired": True, "clarificationRecovered": False,
        "evidenceCompleteness": 1.0, "stableReplay": True,
        "amountAccurate": True, "cleanupComplete": True,
    })
    attempt = next(item for item in recorded["attempts"] if item["id"] == "J01#1")
    assert attempt["status"] == "failed"
    assert recorded["summary"]["thresholds"]["clarificationRecoveryRate"]["actual"] == 0
