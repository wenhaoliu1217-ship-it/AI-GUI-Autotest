export type RunStatus = 'queued' | 'running' | 'waiting_for_clarification' | 'pending_confirmation' | 'passed' | 'issues_found' | 'incomplete' | 'system_error' | 'cancelled' | 'failed' | 'error' | 'pending_review' | 'stopped' | 'skipped';
...
export interface PlanStep {
  ...
  effect_kind?: string;
  effect_level?: 'read_only' | 'session_only' | 'reversible_write' | 'reversible_quota_write' | 'isolated_local_write' | 'sensitive_reversible_write' | 'high_risk_write' | 'high_risk_external_write' | 'high_risk_irreversible' | 'high_risk_public_write' | 'high_risk_identity_write' | 'forbidden';
  target_id?: string;
...
export interface AIConnectionResult {
  ...
  capabilities: {
    schema: 'passed' | 'failed' | 'not_tested';
    agentDecision: 'passed' | 'failed' | 'not_tested';
    multiTurn: 'passed' | 'failed' | 'not_tested';
    vision: 'passed' | 'failed' | 'not_tested';
  };
  visionDetail?: string;
  probeVersion: string;
}
...
export interface TestRun {
  ...
  resultClassification: 'agent_passed' | 'agent_failed' | 'fixed_passed' | 'fixed_failed' | 'fallback_passed' | 'unverified';
  steps: RunStep[];
  ...
  pendingClarification?: {
    id: string; round: number; question: string; requestedAt: string;
  };
  clarificationHistory: Array<{
    id?: string; round: number; question: string; answer: string; actor?: string; answeredAt?: string;
  }>;
...
export interface AcceptanceAttempt {
  ...
  resultClassification: 'agent_passed' | 'agent_failed' | 'fixed_passed' | 'fixed_failed' | 'fallback_passed' | 'unverified';
  clarificationRequired: boolean; clarificationRecovered?: boolean;
}
...
export interface CommerceProfile {
  ...
  fixedProductRef: string | null;
  fixedAddressRef: string | null;
  writtenAuthorizationRef: string | null;
  automaticCancellationVerified: boolean;
...
export interface CesiumAcceptanceCase {
  ...
  effectLevel: string;
  execution: {
    status: 'unverified' | 'blocked' | 'observed_read_only' | 'passed' | 'failed';
...
export interface GAEAcceptanceScenario {
  id: string;
  name: string;
  category: string;
  accountRole: string;
...
export interface GAEAcceptanceCatalog {
  schemaVersion: string;
  scenarioCount: number;
  repeatCount: number;
  plannedRuns: number;
  runtimeBindingSupported: boolean;
  readyCount: number;
  blockedCount: number;
  blockedDependencies: string[];
  scenarios: GAEAcceptanceScenario[];
}

export interface GAEAcceptanceBatch {
  schemaVersion: string;
  batchId: string;
...
export interface GAEL4Workflow {
  id: string;
  name: string;
  bindingStatus: 'ready' | 'blocked';
  stages: Array<{ id: string; goal: string; requiredOutputs: string[] }>;
  successRule: string;
  blockedDependencies: string[];
}

export interface GAEL4RunResult {
  runId: string;
