...
interface BackendStep {
  ...
  browser_context_evidence?: Record<string, unknown>;
  ...
  file_evidence?: { direction: 'upload' | 'download'; sha256: string; bytes: number; assetRef?: string; filename?: string; artifact?: string };
...
interface RecoveryEvidence {
  ...
  backendProbe?: Record<string, unknown>;
  noReplayReason?: string;
}
...
interface BackendRun {
  ...
  role?: string;
  status: RunStatus;
  result_classification?: 'agent_passed' | 'agent_failed' | 'fixed_passed' | 'fixed_failed' | 'fallback_passed' | 'unverified';
  started_at: string;
  ended_at: string;
  steps: BackendStep[];
  assertions: Array<{
  ...
  pending_clarification?: {
    id: string; round: number; question: string; requested_at: string;
  };
  ...
  commerce_summary?: {
    ...
    releaseGate?: import('./types').CommerceReleaseGate;
  };
  findings?: Array<{
...
function toRun(payload: BackendRun): TestRun {
  ...
  const steps = payload.steps.map((step) => ({
    ...
    browserContextEvidence: step.browser_context_evidence,
    commerceStateEvidence: step.commerce_state_evidence,
  ...
  return {
    ...
    resultClassification: payload.result_classification || 'unverified',
    steps,
    ...
    systemError: payload.system_error,
    ...
    pendingClarification: payload.pending_clarification ? {
      id: payload.pending_clarification.id,
      round: payload.pending_clarification.round,
      question: payload.pending_clarification.question,
      requestedAt: payload.pending_clarification.requested_at
    } : undefined,
    clarificationHistory: (payload.clarification_history || []).map((item) => ({
      id: item.id,
      round: item.round,
    ...
    commerceSummary: payload.commerce_summary
  };
}
...
export const api = {
  ...
  resolveWebsite: (url: string) => request<{ url: string; changed: boolean; redirectChain: string[] }>('/api/websites/resolve', {
    method: 'POST', body: JSON.stringify({ url })
  }),
  analyzeWebsiteScope: (projectId: string, settings: AISettings) => request<{ items: string[]; summary: string; source: string }>(`/api/projects/${encodeURIComponent(projectId)}/scope-analysis`, {
    method: 'POST', body: JSON.stringify({ settings })
  }),
  getCesiumAcceptance: () => request<CesiumAcceptanceSuite>('/api/benchmarks/cesium-ion'),
  getGAEAcceptanceCatalog: () => request<GAEAcceptanceCatalog>('/api/acceptance/gaealavic/scenarios'),
  getGAEL4Workflow: () => request<GAEL4Workflow>('/api/acceptance/gaealavic/l4-workflow'),
  getGAEAcceptanceBatches: () => request<GAEAcceptanceBatch[]>('/api/acceptance/gaealavic/batches'),
  startGAEAcceptanceBatch: (payload: { dryRun: boolean; projectId?: string; environmentId?: string; scenarioBindings?: Record<string, unknown> }) => request<GAEAcceptanceBatch>('/api/acceptance/gaealavic/batches', { method: 'POST', body: JSON.stringify(payload) }),
  controlGAEAcceptanceBatch: (batchId: string, action: 'cancel' | 'resume' | 'retry-failed') => request<GAEAcceptanceBatch>(`/api/acceptance/gaealavic/batches/${encodeURIComponent(batchId)}/${action}`, { method: 'POST' }),
  startGAEL4Run: (payload: { dryRun: boolean; projectId?: string; environmentId?: string; stageBindings?: Record<string, unknown> }) => request<GAEL4RunResult>('/api/acceptance/gaealavic/l4-runs', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  scanProject: (projectId: string) => request<CompatibilityReport>(`/api/projects/${encodeURIComponent(projectId)}/scan`, {
    method: 'POST', body: JSON.stringify({ headless: true, timeoutMs: 30_000 })
  }),
  ...
  importSession: (projectId: string, storageState: Record<string, unknown>) => request<SessionMetadata>(`/api/projects/${encodeURIComponent(projectId)}/session`, {
    method: 'POST', body: JSON.stringify({ storageState })
  }),
  getSession: (projectId: string) => request<SessionMetadata>(`/api/projects/${encodeURIComponent(projectId)}/session`),
  startSessionRecording: (projectId: string, timeoutSeconds: number) => request<SessionRecording>(`/api/projects/${encodeURIComponent(projectId)}/session-recordings`, {
    method: 'POST', body: JSON.stringify({ timeoutSeconds })
  }),
  ...
  generatePlan: (draft: TestCaseDraft, projectId?: string, environmentId?: string, scenarioId?: string) => request<PlanGeneration>('/api/plans/generate', {
    method: 'POST',
    body: JSON.stringify({ ...draft, projectId, environmentId, scenarioId })
  }),
  testAI: (settings: AISettings) => request<AIConnectionResult>('/api/ai/probe', {
    method: 'POST',
    body: JSON.stringify({ settings })
  }),
  generateAIPlan: (draft: TestCaseDraft, settings: AISettings, projectId?: string, environmentId?: string, scenarioId?: string) => request<PlanGeneration>('/api/ai/plans/generate', {
    method: 'POST',
    body: JSON.stringify({ draft, settings, projectId, environmentId, scenarioId })
  }),
  ...
  async startRun(plan: TestPlan, projectId?: string, scenarioId?: string, environmentId?: string): Promise<TestRun> {
  ...
  async startAgentRun(draft: TestCaseDraft, settings: AISettings, modelDataAuthorization: { siteHost: string; allowDom: boolean; allowScreenshots: boolean }, projectId?: string, scenarioId?: string, environmentId?: string, enableVisualFallback = false, approvalMode: 'ask' | 'delegate' | 'full' = 'ask'): Promise<TestRun> {
    return toRun(await request<BackendRun>('/api/agent-runs', {
      ...
      body: JSON.stringify({
        ...
        settings,
        modelDataAuthorization: { ...modelDataAuthorization, authorizedBy: 'local_user' },
        headless: false,
        timeoutMs: 30_000,
        projectId,
        scenarioId,
  ...
  async answerClarification(runId: string, clarificationId: string, answer: string): Promise<TestRun> {
    return toRun(await request<BackendRun>(`/api/runs/${encodeURIComponent(runId)}/clarification`, {
      method: 'POST',
      body: JSON.stringify({ clarificationId, answer, actor: 'local_user' })
    }));
  },
  ...
  getFileAssets: (projectId: string) => request<FileAsset[]>(`/api/projects/${encodeURIComponent(projectId)}/file-assets`),
  uploadFileAsset: async (projectId: string, file: File) => {
    const bytes = await file.arrayBuffer();
    const digest = Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', bytes)))
      .map((value) => value.toString(16).padStart(2, '0')).join('');
    const response = await fetch(`${API_BASE}/api/projects/${encodeURIComponent(projectId)}/file-assets`, {
      method: 'POST', body: bytes,
      headers: { 'X-File-Name': file.name, 'X-File-Sha256': digest, 'Content-Type': 'application/octet-stream' }
    });
    if (!response.ok) {
      const payload = await response.json().catch(() => ({})) as { detail?: string };
      throw new Error(payload.detail || `文件登记失败：HTTP ${response.status}`);
    }
  ...
  getAcceptanceBatches: () => request<AcceptanceBatch[]>('/api/acceptance/jd/batches'),
  startAcceptanceBatch: () => request<AcceptanceBatch>('/api/acceptance/jd/batches', { method: 'POST', body: JSON.stringify({}) }),
  getAcceptanceBatch: (batchId: string) => request<AcceptanceBatch>(`/api/acceptance/jd/batches/${encodeURIComponent(batchId)}`),
  controlAcceptanceBatch: (batchId: string, action: 'cancel' | 'resume' | 'retry-failed') => request<AcceptanceBatch>(`/api/acceptance/jd/batches/${encodeURIComponent(batchId)}/${action}`, { method: 'POST', body: JSON.stringify({}) }),
  acceptanceReportUrl: (batchId: string) => `${API_BASE}/api/acceptance/jd/batches/${encodeURIComponent(batchId)}/report.html`,
