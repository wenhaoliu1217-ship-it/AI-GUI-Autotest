...
function cesiumAcceptanceSuite() {
  return {
    suite: 'cesium-ion', version: '1.32.00', target: 'https://ion.cesium.com', inspectedAt: '2026-07-22',
    ...
    cases: [{
      id: 'C10', version: '1.32.00', priority: 'P0', title: '上传合法 GLB', businessGoal: '处理完成',
...
describe('App', () => {
  ...
  it('keeps three ordinary navigation choices and exposes all advanced settings', async () => {
    const user = userEvent.setup();
    render(<App />);
    await screen.findByText('真实执行服务已连接');
    expect(screen.getByLabelText('要测试的网站地址')).toBeVisible();
    expect(screen.getByRole('navigation', { name: '主要导航' }).querySelectorAll('button')).toHaveLength(3);
    ...
    await user.click(screen.getByRole('button', { name: '高级设置' }));
    expect(screen.getByRole('button', { name: /更换 AI 服务/ })).toBeVisible();
    expect(screen.getByRole('main').querySelectorAll('.settings-grid > button')).toHaveLength(7);
  ...
  it('keeps an optional login entry and clears the unrelated old goal for a new website', async () => {
    ...
    render(<App />);
    await screen.findByText('真实执行服务已连接');
    await user.type(screen.getByLabelText('要测试的网站地址'), 'https://example.com');
  ...
  it('configures AI in memory and enables AI planning after a real connection test', async () => {
    ...
    render(<App />);
    await screen.findByText('真实执行服务已连接');
    await openAdvancedPage(user, /更换 AI 服务/);
    const keyInput = screen.getByLabelText('API Key');
    expect(keyInput).toHaveAttribute('type', 'password');
    await user.type(keyInput, 'new-test-key');
    await user.click(screen.getByRole('button', { name: /运行模型能力探针/ }));
    ...
    const request = calls.find(([url]) => String(url).endsWith('/api/ai/probe'));
  ...
  it('continues a completed goal in the same run instead of starting a new run', async () => {
    ...
    expect(await screen.findByText('当前任务已完成')).toBeVisible();
  ...
  it('starts Agent-first exploration from the natural-language goal without a fixed plan', async () => {
    ...
    expect(await screen.findByRole('heading', { name: /输入网址，让 AI 帮你测试/ })).toBeInTheDocument();
    await openAdvancedPage(user, /传统测试编辑器/);
    ...
    expect(payload).toMatchObject({ targetUrl: plan.base_url, enableVisualFallback: true, headless: false });
