...
class ArtifactManager:
    ...
    def finalize(self, result: RunResult) -> None:
        ...
        (self.run_dir / "report.md").write_text(
            self._markdown_report(result), encoding="utf-8"
        )
        (self.run_dir / "report.html").write_text(
            self._html_report(result), encoding="utf-8"
        )
    ...
    def _html_report(self, result: RunResult) -> str:
        ...
        for step in result.steps:
            ...
            step_rows.append(
                ...
                f"<td>{html.escape(error) if error else evidence}</td>"
                "</tr>"
            )
        assertion_rows = []
        for assertion in result.assertions:
...
<section><h2>执行步骤</h2><table><thead><tr><th>#</th><th>动作</th><th>状态</th><th>耗时</th><th>证据 / 错误</th></tr></thead><tbody>{''.join(step_rows)}</tbody></table></section>
