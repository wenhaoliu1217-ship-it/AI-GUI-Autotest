...
from ..security.redaction import summarize_request_url
    ...
    with sync_playwright() as playwright:
        ...
        context.route(
            ...
            lambda route: guard_playwright_route(
                ...
                lambda message, url, resource_type: failed_requests.append(
                    f"BLOCKED {resource_type} {summarize_request_url(url)} ({message[:180]})"
                ),
            ),
        )
        ...
        try:
            while pending and len(visited) < 6:
                ...
                page.on("requestfailed", lambda request: failed_requests.append(f"{request.method} {summarize_request_url(request.url)}"))
                page.on("response", lambda response: http_errors.append(f"HTTP {response.status} {summarize_request_url(response.url)}") if response.status >= 400 else None)
                try:
                    ...
                    if requested == project.base_url:
                        raise
                    failed_requests.append(f"GET {summarize_request_url(requested)} ({(rejection or str(exc))[:160]})")
                finally:
                    page.close()
        finally:
            context.close()
