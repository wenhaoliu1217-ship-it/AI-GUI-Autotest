...
class GAEALaViCCesiumBridgeAdapter(CesiumBridgeAdapter):
    """Validate normalized GAEALaViC semantics without exposing Cesium objects."""
    ...
    def _validate_scene(cls, scene: Any) -> None:
        ...
        if missing:
            raise PlaywrightError(f"GAEALaViC Cesium Adapter 缺少语义状态：{', '.join(missing)}")
        if not isinstance(scene.get("layers"), list):
            raise PlaywrightError("GAEALaViC Cesium Adapter layers 必须为数组")
        for key in ("pathPoints", "pois", "fences", "drawings"):
            if not isinstance(scene.get(key), list):
                raise PlaywrightError(f"GAEALaViC Cesium Adapter {key} 必须为数组")
