        ...
        try:
            ...
            if cfg.business_objects:
                def execute_cleanup(item: dict) -> dict:
                    ...
                    if rule and (cfg.confirmation_callback is None or not cfg.confirmation_callback(cleanup_step, len(executed_steps) + 1, rule)):
                ...
                def verify_cleanup(item: dict) -> bool:
                    ...
                    return True if not raw else resolve_locator(page, Locator.model_validate(raw)).count() == 0
                ...
                if cleanup_report["status"] == "failed":
                    ...
                    hints.append(_cause_hint(FailureCategory.CLEANUP, len(executed_steps) + 1, "业务测试对象清理失败"))
        finally:
            try:
