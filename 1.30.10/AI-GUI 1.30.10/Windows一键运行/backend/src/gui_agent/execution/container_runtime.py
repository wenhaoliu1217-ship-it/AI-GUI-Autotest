...
DEFAULT_RUNNER_IMAGE = "ai-gui-runner:1.30.10"
...
class DockerRunHandle:
    def __init__(
        ...
        *,
        ...
        staged_files = self._stage_test_files(config.test_files, run_id)
        ...
        spec = build_container_spec(plan, config)
        spec["config"]["test_files"] = staged_files
        self.send(spec)
    ...
    def cleanup_staged_files(self) -> None:
        directory = (self.run_dir / "_runner_inputs").resolve()
        if directory.parent == self.run_dir.resolve() and directory.is_dir():
    ...
    def _stage_test_files(self, records: tuple[dict, ...], run_id: str) -> list[dict]:
        staged: list[dict] = []
        directory = self.run_dir / "_runner_inputs"
        for record in records:
            source = Path(str(record.get("path", ""))).resolve()
            if not source.is_file():
                raise RuntimeError(f"测试文件暂存不存在：{record.get('id')}")
            file_id = str(record.get("id", ""))
            extension = str(record.get("extension", ""))
            if not file_id.startswith("file-") or any(char not in "abcdefghijklmnopqrstuvwxyz0123456789-." for char in file_id + extension):
                raise RuntimeError("测试文件登记信息非法")
            directory.mkdir(parents=True, exist_ok=True)
            target = directory / f"{file_id}{extension}"
            shutil.copyfile(source, target)
            staged.append({
                **{key: value for key, value in record.items() if key != "path"},
                "path": f"/work/artifacts/{run_id}/_runner_inputs/{target.name}",
            })
        return staged
