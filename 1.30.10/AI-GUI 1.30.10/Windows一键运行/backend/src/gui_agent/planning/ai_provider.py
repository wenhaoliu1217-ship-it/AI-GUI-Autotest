...
def _planning_prompt(
    *,
    ...
    return (
        ...
        "优先 label、role+name、placeholder、test_id、稳定 data 属性、text，最后才使用 CSS；"
        "列表行、卡片、弹窗、页签面板或 Canvas 内动作必须使用 scope 和业务 identity；"
        "禁止在多个匹配中默认选择第一个；不得输出测试成功/失败结论；"
        ...
        "blockedItems 中的事实不得使用；复杂组件只能原样引用 status=configured 的 componentAdapters，并填写 component_adapter_id，不得临时编造目标站选择器；"
