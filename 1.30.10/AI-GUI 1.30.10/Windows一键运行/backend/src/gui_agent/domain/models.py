...
class LocatorScope(BaseModel):
    ...
    identity: Optional[str] = Field(
        ...
        description="作用域必须包含的业务对象名称或 ID",
    )


class Locator(BaseModel):
    """确定性定位器，可先锁定业务容器再在容器内解析目标。"""
    ...
    def _at_least_one_strategy(self) -> "Locator":
        if not any([self.role, self.label, self.placeholder, self.test_id, self.attribute, self.css, self.text]):
...
class ComponentAction(BaseModel):
    """A target-independent semantic contract for common complex Web components."""

    model_config = {"extra": "forbid", "populate_by_name": True}

    kind: Literal[
        "cascade_select", "searchable_select", "date_time_range", "pagination",
        "statistics_card", "tab", "upload_dialog", "image_preview", "local_scroll",
    ]
    ...
    def validate_contract(self) -> "ComponentAction":
        counts = {
            "cascade_select": (1, 1), "searchable_select": (3, 1),
            "date_time_range": (2, 2), "pagination": (1, 0),
            "statistics_card": (1, 0), "tab": (1, 0),
            "upload_dialog": (2, 0), "image_preview": (2, 0), "local_scroll": (1, 0),
        }
        minimum_locators, minimum_values = counts[self.kind]
        if len(self.locators) < minimum_locators or len(self.values) < minimum_values:
...
class Step(BaseModel):
    ...
    def _validate_action_shape(self) -> "Step":
        ...
        if self.action == ActionType.NAVIGATE:
            ...
            if not self.locator or self.download_validation is None:
                raise ValueError("download 需要触发 locator 和 download_validation")
            if not self.business_object_name or not self.business_object_name.startswith("E2E_"):
                raise ValueError("download 的业务对象名称必须使用 E2E_ 前缀")
            ...
            if self.component is None:
                raise ValueError("component 动作需要复杂组件语义配置")
            if self.locator is not None:
                raise ValueError("component 动作的定位器必须封装在 component.locators 中")
            if self.component.kind == "upload_dialog" and (
                not self.business_object_name or not self.business_object_name.startswith("E2E_")
            ):
                raise ValueError("上传弹窗组件需要 E2E_ 业务对象名称")
...
class Assertion(BaseModel):
    ...
    def _validate_assertion_shape(self) -> "Assertion":
        ...
        semantic_counts = {
            AssertionType.CANVAS_ENTITY_COUNT, AssertionType.CANVAS_PATH_POINT_COUNT,
            AssertionType.CANVAS_POI_COUNT, AssertionType.CANVAS_FENCE_COUNT,
            AssertionType.CANVAS_DRAWING_COUNT,
        }
