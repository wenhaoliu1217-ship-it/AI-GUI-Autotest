...
也可以双击 `启动真实GUI测试.cmd`。需要先安装并启动 Docker Desktop（WSL2 Linux Engine）；启动器会构建 `ai-gui-runner:1.30.10`，Docker 不可用时拒绝降级。首次运行还会创建 `.venv-real` 并安装项目扫描所需的宿主 Chromium。
...
- 当前工作目录是 `1.30.10` 开发版；只有 ER-07、首个企业测试网站适配和企业 Definition of Done 全部通过后，才能声明 `1.30.10` 已验收并正式发布。
