...
def test_demo_flow_end_to_end(tmp_path: Path, monkeypatch) -> None:
    ...
    assert (run_dir / "evidence" / "evidence-manifest.json").exists()
    assert result.evidence_manifest_path == "evidence/evidence-manifest.json"
    assert result.evidence_completeness >= 0.95
    assert result.evidence_manifest
    assert result.evidence_manifest["missingCount"] == 0
