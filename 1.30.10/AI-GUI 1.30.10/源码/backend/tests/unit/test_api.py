...
def test_gaealavic_cesium_adapter_is_downloadable() -> None:
    response = client.get("/api/bridge/gaealavic-cesium-adapter")
    assert response.status_code == 200
    assert "installGAEALaViCCesiumTestBridge" in response.text
    assert "getBusinessSceneState" in response.text
    assert "getSceneState" in response.text and "waitForSceneReady" in response.text


def test_acceptance_dry_run_batch_has_progress_controls_and_downloads(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(server, "ACCEPTANCE_BATCHES", AcceptanceBatchManager(tmp_path / "batches"))
    response = client.post("/api/acceptance/batches", json={"dryRun": True})
    assert response.status_code == 200, response.text
    batch_id = response.json()["batchId"]

    deadline = monotonic() + 10
    while monotonic() < deadline:
        payload = client.get(f"/api/acceptance/batches/{batch_id}").json()
        if payload["status"] == "completed":
            break
        sleep(0.02)
    assert payload["completedRuns"] == 150
    assert payload["dryRun"] is True
    assert client.get(f"/api/acceptance/batches/{batch_id}/summary.json").status_code == 200
    assert client.get(f"/api/acceptance/batches/{batch_id}/report.md").status_code == 200
    assert client.post(f"/api/acceptance/batches/{batch_id}/cancel").status_code == 409


def test_l4_dry_run_is_unverified_and_downloadable(tmp_path: Path, monkeypatch) -> None:
    root = (tmp_path / "l4-runs").resolve()
    root.mkdir()
    monkeypatch.setattr(server, "L4_RUNS_ROOT", root)

    response = client.post("/api/acceptance/l4-runs", json={"dryRun": True})
    assert response.status_code == 200, response.text
    payload = response.json()
    assert payload["status"] == "unverified"
    assert payload["verificationStatus"] == "dry_run_only"
    assert client.get(payload["reportUrls"]["json"]).status_code == 200
    assert client.get(payload["reportUrls"]["markdown"]).status_code == 200
...
def test_project_session_is_domain_checked_and_dpapi_encrypted(tmp_path: Path, monkeypatch) -> None:
    ...
    encrypted = (tmp_path / "projects" / project["id"] / "sessions" / "default.dpapi").read_bytes()
...
def test_project_keeps_independent_encrypted_sessions_for_account_roles(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(server, "PROJECT_STORE", ProjectStore(tmp_path / "projects"))
    project = client.post("/api/projects", json={
        "name": "角色矩阵", "baseUrl": "https://example.com",
        "accountProfiles": [
            {"id": "admin", "name": "管理员", "role": "admin", "loginMethod": "interactive", "permissions": ["管理建模"]},
            {"id": "readonly", "name": "只读账号", "role": "readonly", "loginMethod": "credentials", "credentialRefs": {"username": "READONLY_USER", "password": "READONLY_PASSWORD"}},
        ],
    }).json()
    state = {"cookies": [{"name": "session", "value": "secret", "domain": "example.com", "path": "/"}], "origins": []}

    for account_id in ("admin", "readonly"):
        response = client.post(
            f"/api/projects/{project['id']}/session",
            json={"storageState": state, "accountId": account_id},
        )
        assert response.status_code == 200, response.text
        assert response.json()["accountId"] == account_id

    sessions = client.get(f"/api/projects/{project['id']}/sessions").json()
    assert [item["accountId"] for item in sessions] == ["admin", "readonly"]
    assert (tmp_path / "projects" / project["id"] / "sessions" / "admin.dpapi").is_file()
    assert (tmp_path / "projects" / project["id"] / "sessions" / "readonly.dpapi").is_file()
    assert "secret" not in str(sessions)
    deleted = client.delete(f"/api/projects/{project['id']}/session?accountId=readonly")
    assert deleted.status_code == 200
    assert [item["accountId"] for item in client.get(f"/api/projects/{project['id']}/sessions").json()] == ["admin"]
...
def test_compatibility_scan_creates_an_editable_sample_scenario(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_scan(saved_project, **_kwargs):
        return CompatibilityReport(
            ...
            scanMode="low_risk",
            appMap={"version": "1", "projectId": saved_project.id, "generatedAt": "2026-07-22T00:00:00Z", "scanMode": "low_risk", "pages": [{"id": "page-1", "url": saved_project.base_url, "title": "历史应用", "probes": []}]},
        )

    monkeypatch.setattr(server, "scan_project", fake_scan)
    response = client.post(f"/api/projects/{project['id']}/scan", json={"headless": True, "timeoutMs": 30000, "mode": "low_risk"})
    ...
    app_map = client.get(f"/api/projects/{project['id']}/app-map")
    assert app_map.status_code == 200
    assert app_map.json()["pages"][0]["id"] == "page-1"
...
def test_interactive_login_recording_uses_existing_secure_session_contract(tmp_path: Path, monkeypatch) -> None:
    ...
    class FakeManager:
        def start(self, saved_project, store, timeout, account_id):
...
def test_project_test_file_manifest_tracks_valid_and_negative_samples(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(server, "PROJECT_STORE", ProjectStore(tmp_path / "projects"))
    project = client.post("/api/projects", json={"name": "File project", "baseUrl": "https://example.com"}).json()
    content = b'{"name":"E2E_agent"}'

    valid = client.post(
        f"/api/projects/{project['id']}/test-files",
        params={"fileName": "E2E_agent.json", "expectedResult": "accepted", "validationProfile": "json"},
        content=content, headers={"Content-Type": "application/json"},
    )
    invalid = client.post(
        f"/api/projects/{project['id']}/test-files",
        params={"fileName": "E2E_agent.exe", "expectedResult": "rejected", "validationProfile": "auto"},
        content=b"not executable", headers={"Content-Type": "application/octet-stream"},
    )

    assert valid.status_code == 200 and valid.json()["validationStatus"] == "valid"
    assert valid.json()["sha256"] == hashlib.sha256(content).hexdigest()
    assert invalid.status_code == 200 and invalid.json()["validationStatus"] == "invalid"
    assert "不支持的文件扩展名" in invalid.json()["validationErrors"][0]
    manifest = client.get(f"/api/projects/{project['id']}/test-files").json()
    assert {item["id"] for item in manifest} == {valid.json()["id"], invalid.json()["id"]}
    manifest_path = tmp_path / "projects" / project["id"] / "test-data-manifest.json"
    assert manifest_path.is_file()
    assert all("content" not in item and "path" not in item for item in json.loads(manifest_path.read_text(encoding="utf-8")))

    deleted = client.delete(f"/api/projects/{project['id']}/test-files/{invalid.json()['id']}")
    assert deleted.status_code == 200
    assert len(client.get(f"/api/projects/{project['id']}/test-files").json()) == 1


def test_file_action_requires_selected_project_environment(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(server, "PROJECT_STORE", ProjectStore(tmp_path / "projects"))
    project = client.post("/api/projects", json={"name": "File project", "baseUrl": "https://example.com"}).json()
    record = client.post(
        f"/api/projects/{project['id']}/test-files", params={"fileName": "E2E_agent.json"},
        content=b'{"name":"E2E_agent"}', headers={"Content-Type": "application/json"},
    ).json()
    plan = {
        "name": "Upload", "base_url": "https://example.com", "steps": [
            {"action": "upload", "locator": {"label": "Agent JSON"}, "file_id": record["id"], "business_object_name": "E2E_Agent"}
        ],
    }

    response = client.post("/api/runs", json={"plan": plan, "projectId": project["id"], "asyncExecution": True})

    assert response.status_code == 422
    assert "项目测试环境" in response.json()["detail"]
