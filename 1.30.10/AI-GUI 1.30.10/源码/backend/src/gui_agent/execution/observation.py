...
class ObservationCollector:
            ...
            result = self.page.evaluate(
                """() => {
                  ...
                  const targetName = (el) => {
                    ...
                    const stableAttribute = ['data-object-id','data-test','data-qa','data-cy']
                      .map(name => [name, el.getAttribute(name)]).find(([, value]) => value);
                    const text = (el.innerText || el.textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 100);
                    return [tag, role && `role=${role}`, label && `label=${label}`,
                      testId && `testid=${testId}`, placeholder && `placeholder=${placeholder}`,
                      stableAttribute && `${stableAttribute[0]}=${stableAttribute[1]}`,
                      text && `text=${text}`].filter(Boolean).join(' | ');
                  };
    ...
    def _dom_summary(self) -> list[str]:
        try:
            result = self.page.evaluate(
                """() => {
                  const selectors = 'a,button,input,select,textarea,[role],[aria-label],[data-testid],[data-test],[data-qa],[data-cy],[data-object-id],h1,h2,h3';
                  return Array.from(document.querySelectorAll(selectors)).slice(0, 80).map((node) => {
                    ...
                    return [tag, role && `role=${role}`, label && `label=${label}`,
                      testId && `testid=${testId}`, placeholder && `placeholder=${placeholder}`,
                      stableAttribute && `${stableAttribute[0]}=${stableAttribute[1]}`, type && `type=${type}`,
