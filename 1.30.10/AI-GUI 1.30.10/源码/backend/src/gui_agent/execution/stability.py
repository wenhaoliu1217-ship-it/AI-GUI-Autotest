...
from ..locating.strategies import LocatorError, resolve_action_locator, resolve_locator
...
def prepare_action(
    ...
    *,
    ...
    visual_locator = step.canvas_region_locator or step.locator
    if step.action in LOCATOR_ACTIONS or (step.action in VISUAL_ACTIONS and visual_locator is not None):
        locator_model = step.locator if step.action in LOCATOR_ACTIONS else visual_locator
        assert locator_model is not None
        if step.action == ActionType.WAIT_FOR:
            locator = resolve_locator(page, locator_model)
            locator.wait_for(state="visible", timeout=timeout_ms)
            count = locator.count()
            if count != 1:
                raise LocatorError(f"等待目标出现后必须唯一，实际匹配 {count} 个：{locator_model.describe()}")
        else:
            locator = resolve_action_locator(page, locator_model)
            locator.wait_for(state="visible", timeout=timeout_ms)
        require_enabled = step.action not in {ActionType.WAIT_FOR, ActionType.WAIT_FOR_STATE, ActionType.HOVER, ActionType.VISUAL_HOVER}
        require_unoccluded = step.action not in {ActionType.WAIT_FOR, ActionType.WAIT_FOR_STATE}
...
def finalize_canvas_evidence(
    ...
    *,
    ...
    evidence.update({
        ...
        "gestureEvidence": execution_detail.get("gestureEvidence"),
    })
    return evidence
