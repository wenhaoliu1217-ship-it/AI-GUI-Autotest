...
def _build_canvas_findings(step: StepResult, reproduction: list[str]) -> list[Finding]:
    ...
    visual_unchanged = (
        mode == "visual"
        and evidence.get("action") in {"visual_click", "visual_drag", "visual_zoom", "visual_clear", "visual_draw_polygon", "visual_draw_rectangle"}
...
def build_findings(steps: list[StepResult], assertions: list[AssertionResult], reproduction: list[str]) -> list[Finding]:
    ...
    for assertion in assertions:
        if assertion.status != Status.PASSED:
            semantic = assertion.semantic_evidence or {}
            semantic_categories = {
                "canvas_layer_visible": ("canvas_layer_mismatch", "图层可见性不符合预期"),
                "canvas_camera_equals": ("canvas_camera_mismatch", "相机状态不符合预期"),
                "canvas_entity_count": ("canvas_entity_mismatch", "实体数量不符合预期"),
                "canvas_selected_entity": ("canvas_selection_mismatch", "选中实体不符合预期"),
                "canvas_path_point_count": ("canvas_path_mismatch", "路径点数量不符合预期"),
                "canvas_poi_count": ("canvas_poi_mismatch", "POI 数量不符合预期"),
                "canvas_fence_count": ("canvas_fence_mismatch", "围栏数量不符合预期"),
                "canvas_drawing_count": ("canvas_drawing_mismatch", "绘制对象数量不符合预期"),
                "canvas_tiles_loaded": ("canvas_tiles_not_loaded", "瓦片未加载完成"),
                "canvas_webgl_no_error": ("canvas_webgl_error", "检测到 WebGL 错误"),
            }
            semantic_type = semantic.get("type")
            if semantic_type in semantic_categories:
                category, title = semantic_categories[semantic_type]
                facts = [
                    _fact("assertionType", semantic_type),
                    _fact("classification", semantic.get("classification")),
                    _fact("sceneState", semantic.get("sceneState")),
                ]
                findings.append(Finding(
                    id=f"finding-{uuid4().hex[:10]}", title=title, category=category,
                    severity="High", confidence="high",
                    actual_result=assertion.actual_summary or assertion.error_message or "无法获得语义状态",
                    expected_result=assertion.expected_summary or assertion.description or assertion.type,
                    facts=facts,
                    inference=f"失败域为 {semantic.get('classification', 'ai_cannot_determine')}；不能仅凭该断言判定产品根因，需结合测试数据、权限、环境和时序证据复核。",
