"""Read-only and explicitly low-risk application mapping."""
...
from .models import CompatibilityReport, ProjectConfig, utc_now
...
_MUTATING_UI = _DANGEROUS_NAVIGATION + (
    "save", "submit", "upload", "import", "create", "add", "start", "stop", "restart", "run",
    "保存", "提交", "上传", "导入", "创建", "新增", "启动", "停止", "重启", "运行", "批量",
)
...
def _safe_probe_label(label: str) -> bool:
    lowered = label.lower()
    return bool(label.strip()) and not any(token in lowered for token in _MUTATING_UI)


def _inspect_page(page: Page, url: str, timeout_ms: int, *, navigate: bool = True) -> tuple[dict, list[str]]:
    redirect_chain: list[str] = []
    response = page.goto(url, wait_until="commit") if navigate else None
    ...
    facts = page.evaluate(
        ...
        () => {
          ...
          const compact = values => Array.from(new Set(values.filter(Boolean))).slice(0, 80);
          const describe = el => ({
          ...
          const inputs = Array.from(document.querySelectorAll('input,textarea')).map(describe);
          const selects = Array.from(document.querySelectorAll('select,[role="combobox"]')).map(describe);
          const tabs = Array.from(document.querySelectorAll('[role="tab"]')).map(describe);
          const dialogs = Array.from(document.querySelectorAll('dialog,[role="dialog"]')).map(describe);
          const tableColumns = compact(Array.from(document.querySelectorAll('th,[role="columnheader"]')).map(nameOf));
          const cardFields = compact(Array.from(document.querySelectorAll('[class*="card" i] [data-testid],[role="listitem"] [data-testid]')).map(nameOf));
          const probeCandidates = Array.from(document.querySelectorAll('[role="tab"],nav button,aside button,header button,button[aria-controls]'))
            .map(el => ({ role: el.getAttribute('role') || 'button', name: nameOf(el).slice(0, 160) }))
          ...
          return {
            ...
            locators: {
              ...
              placeholders: document.querySelectorAll('input[placeholder],textarea[placeholder]').length,
              stableAttributes: document.querySelectorAll('[data-test],[data-qa],[data-cy],[data-object-id]').length,
            ...
            resources, headings: text('h1,h2,h3'), nav, navLabels, probeCandidates,
            appMap: {
              route: location.pathname + location.search,
              pageName: document.title || text('h1')[0] || '',
              mainRegions: compact(Array.from(document.querySelectorAll('main,[role="main"],section')).map(nameOf)),
              menuItems: compact(navLabels), tabs, buttons, inputs, selects,
              fileUploads: inputs.filter(item => item.type === 'file'),
              tableColumns, cardFields, dialogs,
              canvases: canvases.map((el, index) => ({ id: el.id || `canvas-${index + 1}`, width: el.width, height: el.height })),
              iframes: Array.from(document.querySelectorAll('iframe')).map(describe),
              shadowRootCount: all.filter(el => el.shadowRoot).length,
              unlabeledControlCount: unlabeledControls,
              highRiskActions: compact(buttons.map(item => item.name).filter(name => /删除|支付|发布|邀请|启动|停止|重启|delete|pay|publish|invite|start|stop|restart/i.test(name))),
            },
...
def scan_project(
    ...
    *,
    ...
    scan_mode: Literal["read_only", "low_risk"] = "read_only",
) -> CompatibilityReport:
    policy = DomainPolicy(
        project.base_url,
    ...
    with sync_playwright() as playwright:
        ...
        def scan_route(route) -> None:
            if route.request.method.upper() not in {"GET", "HEAD", "OPTIONS"}:
                failed_requests.append(f"BLOCKED_MUTATION {route.request.method} {route.request.url[:350]}")
            ...
            while pending and len(visited) < 6:
                ...
                try:
                    ...
                    profile = {
                        ...
                        "map": facts["appMap"], "probes": [],
                    }
                    if scan_mode == "low_risk":
                        for candidate in facts.get("probeCandidates", [])[:12]:
                            if not _safe_probe_label(candidate.get("name", "")):
                                continue
                            probe = context.new_page()
                            try:
                                probe.goto(facts["finalUrl"], wait_until="domcontentloaded")
                                target = probe.get_by_role(
                                    candidate.get("role") or "button", name=candidate["name"], exact=True
                                )
                                if target.count() != 1:
                                    continue
                                target.click(timeout=min(timeout_ms, 5_000))
                                probe.evaluate(
                                    "() => new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)))"
                                )
                                probe_facts, _ = _inspect_page(probe, probe.url, timeout_ms, navigate=False)
                                profile["probes"].append({
                                    "action": "click", "role": candidate.get("role"), "name": candidate["name"],
                                    "resultUrl": probe_facts["finalUrl"], "map": probe_facts["appMap"],
                                })
                            except Exception as exc:
                                profile["probes"].append({
                                    "action": "click", "name": candidate["name"], "error": str(exc)[:300],
                                })
                            finally:
                                probe.close()
    ...
    app_map = {
        ...
        "pages": [
            {
                "id": f"page-{index + 1}", "url": item["url"], "title": item["title"],
                "pageType": item["pageType"], **item.get("map", {}), "probes": item.get("probes", []),
            }
