...
$runnerImage = 'ai-gui-runner:1.30.20'
...
if (-not $env:GUI_TEST_DATA_ROOT) {
  $defaultTestData = Join-Path $projectRoot 'test-data'
  if (Test-Path -LiteralPath $defaultTestData -PathType Container) {
    $env:GUI_TEST_DATA_ROOT = $defaultTestData
  }
}
