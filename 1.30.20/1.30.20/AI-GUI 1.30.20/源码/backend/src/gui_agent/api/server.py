...
app = FastAPI(title="京彩OPC AI GUI 执行服务", version="1.30.20")
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        origin.strip()
...
def _configured_test_data_root() -> Path | None:
    """Resolve the optional, read-only upload fixture repository at run creation time."""
    configured = os.getenv("GUI_TEST_DATA_ROOT", "").strip()
...
async def execute_run(payload: RunRequest) -> dict:
    ...
    config = RunnerConfig(
        ...
        test_data_root=_configured_test_data_root(),
...
async def replay_run(run_id: str, payload: ReplayRequest) -> dict:
    ...
    replay_base_url = resolve_env_placeholder(plan.base_url, replay_environment.variables if replay_environment else None)
