...
class EffectLevel(str, Enum):
    """Declared business side effect used by target-specific execution policy gates."""
...
class Locator(BaseModel):
    ...
    placeholder: Optional[str] = Field(default=None, description="输入控件 placeholder")
    attribute_name: Optional[str] = Field(default=None, description="原生 name 属性")
    test_id: Optional[str] = Field(default=None, description="data-testid 值")
    href: Optional[str] = Field(default=None, description="链接的稳定 href")
    ...
    shadow_hosts: list[str] = Field(
        ...
        description="从外到内的 open Shadow DOM host CSS 路径",
    )
    within: Optional["Locator"] = Field(
        default=None,
        description="先定位唯一行、卡片或容器，再在其内部解析当前目标",
    )
    ...
    def describe(self) -> str:
        ...
        if self.within:
            parts.insert(0, f"within=({self.within.describe()})")
        return ", ".join(parts)


class WaitCondition(BaseModel):
    """业务状态等待条件，不使用固定 sleep 作为成功依据。"""

    model_config = {"extra": "forbid"}

    source: str = Field(pattern="^(text|url|attribute|bridge)$")
    expected: str
    locator: Optional[Locator] = None
    attribute: Optional[str] = None
    timeout_ms: int = Field(default=120_000, ge=1_000, le=7_200_000)
    interval_ms: int = Field(default=1_000, ge=100, le=30_000)

    @model_validator(mode="after")
    def _validate_condition(self) -> "WaitCondition":
        if self.source in {"text", "attribute"} and not self.locator:
            raise ValueError(f"{self.source} 状态等待需要 locator")
        if self.source == "attribute" and not self.attribute:
            raise ValueError("attribute 状态等待需要 attribute")
        return self
...
class Step(BaseModel):
    ...
    file_ids: list[str] = Field(
        default_factory=list,
        description="测试文件仓库中登记的相对路径或文件 ID",
    )
    download_name_pattern: Optional[str] = Field(
        default=None,
        description="下载建议文件名必须满足的正则表达式",
    )
    wait_condition: Optional[WaitCondition] = None
    effect_kind: Optional[str] = Field(
        default=None,
        description="目标站点策略表中的副作用类型；Cesium ion 运行必须显式声明",
    )
    effect_level: Optional[EffectLevel] = Field(
        default=None,
        description="副作用等级，必须与目标站点策略定义一致",
    )
    ...
    account_context: Optional[str] = Field(default=None, description="个人账号或 Team 上下文")
    ...
    def _validate_action_shape(self) -> "Step":
        ...
        elif self.action == ActionType.UPLOAD_FILES:
            if not self.locator or not self.file_ids:
                raise ValueError("upload_files 需要文件输入定位器和至少一个 file_id")
        elif self.action == ActionType.DOWNLOAD:
            if not self.locator:
                raise ValueError("download 需要触发下载的 locator")
        elif self.action == ActionType.WAIT_UNTIL:
            if not self.wait_condition:
                raise ValueError("wait_until 需要 wait_condition")
