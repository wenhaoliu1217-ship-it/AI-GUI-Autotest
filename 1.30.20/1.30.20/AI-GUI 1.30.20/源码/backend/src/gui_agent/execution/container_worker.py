...
def _config(spec: dict, cancel_event: Event, decisions: queue.Queue) -> RunnerConfig:
    ...
    if payload.get("test_data_root"):
        payload["test_data_root"] = Path(payload["test_data_root"])
    tuple_fields = {
        "allowed_hosts", "forbidden_actions", "environment_variables", "secret_refs",
        "ignore_rules", "screenshot_mask_selectors", "viewport", "cesium_owned_resources",
    }
    ...
    def receive() -> None:
        for line in sys.stdin:
            ...
            if message.get("type") == "cancel":
                cancel_event.set()
            elif message.get("type") == "confirmation_decision":
