...
DEFAULT_RUNNER_IMAGE = "ai-gui-runner:1.30.20"
...
def build_container_spec(plan: TestPlan, config: RunnerConfig) -> dict:
    ...
    for item in fields(RunnerConfig):
        ...
        if isinstance(value, Path):
            value = str(value)
        if item.name == "test_data_root" and value:
            value = "/work/test-data"
...
def build_docker_command(
    ...
    if config.test_data_root is not None:
        test_data_root = Path(config.test_data_root).resolve()
        if not test_data_root.is_dir():
            raise RuntimeError("受控测试文件仓库不存在，拒绝启动容器 Runner")
        command.extend([
            "--mount",
            f"type=bind,src={test_data_root},dst=/work/test-data,readonly",
        ])
...
class DockerRunHandle:
    def __init__(
        ...
        *,
        ...
        self.send(build_container_spec(plan, config))
