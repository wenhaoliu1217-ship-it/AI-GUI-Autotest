            ...
            for index, step in enumerate(plan.steps, start=1):
                ...
                try:
                    ...
                    execution_detail = _execute_step(
                        ...
                        artifacts=artifacts, test_data_root=cfg.test_data_root,
                    )
                    ...
                    result = StepResult(
                        ...
                        async_timeline=execution_detail.get("asyncTimeline", []),
                        before=before,
                        after=after,
                    )
                    steps.append(result)
...
def _execute_step(
    ...
    *, environment_variables: dict[str, str] | None = None, secret_refs: dict[str, str] | None = None,
    ...
    if step.action == ActionType.UPLOAD_FILES:
        if test_data_root is None:
            raise SecurityError("上传动作未配置受控测试文件仓库")
        files, evidence = _resolve_test_files(test_data_root, step.file_ids)
        locator = resolve_locator(page, step.locator)  # type: ignore[arg-type]
        locator.first.set_input_files([str(path) for path in files])
        return {"fileEvidence": evidence}
    ...
    if step.action == ActionType.DOWNLOAD:
        ...
        if safe_name != suggested or safe_name in {"", ".", ".."}:
            raise SecurityError("下载文件名包含路径穿越或无效名称")
        if step.download_name_pattern and not re.fullmatch(step.download_name_pattern, safe_name):
            raise PlaywrightError(f"下载文件名不符合预期：{safe_name}")
        target = _unique_file_path(artifacts.run_dir / "downloads" / safe_name)
        target.parent.mkdir(parents=True, exist_ok=True)
        download.save_as(str(target))
        return {"fileEvidence": [_file_evidence(target, artifacts.run_dir)]}

    if step.action == ActionType.WAIT_UNTIL:
        condition = step.wait_condition
        assert condition is not None
        deadline = monotonic() + condition.timeout_ms / 1000
        timeline: list[dict] = []
        while monotonic() < deadline:
            observed = _read_wait_value(page, condition, bridge_adapter)
            timeline.append({"observed": redactor.scrub(observed)[:1000], "elapsedMs": int((condition.timeout_ms / 1000 - max(0, deadline - monotonic())) * 1000)})
            if condition.expected in observed:
                return {"asyncTimeline": timeline}
            page.wait_for_timeout(condition.interval_ms)
        raise PlaywrightTimeoutError(
            f"业务状态等待超时：期望 {redactor.scrub(condition.expected)}，最后状态 {timeline[-1]['observed'] if timeline else '无'}"
        )
...
def _resolve_test_files(root: Path, file_ids: list[str]) -> tuple[list[Path], list[dict]]:
    resolved_root = root.resolve()
    if not resolved_root.is_dir():
        raise SecurityError("测试文件仓库不存在")
    files: list[Path] = []
    evidence: list[dict] = []
    for file_id in file_ids:
        relative = Path(file_id)
        if relative.is_absolute() or ".." in relative.parts:
            raise SecurityError(f"测试文件引用越出仓库：{file_id}")
        candidate = (resolved_root / relative).resolve()
        try:
            candidate.relative_to(resolved_root)
        except ValueError as exc:
            raise SecurityError(f"测试文件引用越出仓库：{file_id}") from exc
        if not candidate.exists():
            raise SecurityError(f"测试文件未登记或不存在：{file_id}")
        if candidate.is_dir():
            directory_files = sorted(path for path in candidate.rglob("*") if path.is_file())
            if not directory_files:
                raise SecurityError(f"测试目录为空：{file_id}")
            files.append(candidate)
            evidence.extend(_file_evidence(path, resolved_root) for path in directory_files)
        else:
            files.append(candidate)
            evidence.append(_file_evidence(candidate, resolved_root))
    return files, evidence


def _file_evidence(path: Path, relative_to: Path) -> dict:
    ...
    if zipfile.is_zipfile(path):
        with zipfile.ZipFile(path) as archive:
            ...
            if unsafe:
                raise SecurityError("ZIP 包含路径穿越条目")
            entry["archiveEntries"] = names[:500]
    return entry
...
def _unique_file_path(path: Path) -> Path:
    ...
    for index in range(2, 10_000):
        ...
        if not candidate.exists():
            return candidate
    raise SecurityError("下载目录中同名文件过多，拒绝覆盖既有证据")


def _read_wait_value(page, condition, bridge_adapter: CanvasAppBridgeAdapter | None) -> str:
    if condition.source == "url":
        return page.url
    if condition.source == "bridge":
        if bridge_adapter is None:
            raise SecurityError("Bridge 状态等待要求环境启用 App Bridge")
        return json.dumps(bridge_adapter.capture_state(page, phase="poll"), ensure_ascii=False, default=str)
    locator = resolve_locator(page, condition.locator)  # type: ignore[arg-type]
    if condition.source == "attribute":
        return locator.first.get_attribute(condition.attribute or "") or ""
...
def _validate_runtime_cesium_step(
    step: Step, index: int, owned_resources: tuple[tuple[str, str, str], ...]
) -> None:
    entries = [
