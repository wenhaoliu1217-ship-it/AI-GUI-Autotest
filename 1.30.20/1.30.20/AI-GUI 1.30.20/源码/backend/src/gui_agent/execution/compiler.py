...
def _compile_step(step: Step, base_url: str) -> list[str]:
    ...
    if step.action == ActionType.UPLOAD_FILES:
        file_ids = json.dumps(step.file_ids, ensure_ascii=False)
        return [
            "const testDataRoot = process.env.GUI_TEST_DATA_ROOT;",
            "if (!testDataRoot) throw new Error('GUI_TEST_DATA_ROOT is required for upload_files');",
            f"const uploadFiles = {file_ids}.map((fileId) => path.resolve(testDataRoot, fileId));",
            f"await {_locator(step.locator)}.setInputFiles(uploadFiles);",
        ]
    if step.action == ActionType.DOWNLOAD:
        lines = [
            "const [download] = await Promise.all([",
            "  page.waitForEvent('download'),",
            f"  {_locator(step.locator)}.click(),",
            "]);",
        ]
        if step.download_name_pattern:
            lines.append(
                f"expect(download.suggestedFilename()).toMatch(new RegExp({json.dumps(step.download_name_pattern)}));"
            )
        lines.extend([
            "const downloadRoot = process.env.GUI_DOWNLOAD_ROOT || test.info().outputPath('downloads');",
            "await mkdir(downloadRoot, { recursive: true });",
            "await download.saveAs(path.join(downloadRoot, path.basename(download.suggestedFilename())));",
        ])
        return lines
    if step.action == ActionType.WAIT_UNTIL:
        ...
        if condition.source == "url":
            observed = "page.url()"
        elif condition.source == "attribute":
            observed = f"(await {_locator(condition.locator)}.getAttribute({json.dumps(condition.attribute or '')})) || ''"
        elif condition.source == "bridge":
            observed = (
                "JSON.stringify(await page.evaluate(async () => { "
                "const bridge = (window as any)[process.env.APP_BRIDGE_GLOBAL || '__WEB_AI_TEST__']; "
                "if (!bridge || typeof bridge.getSceneState !== 'function') throw new Error('App Bridge is unavailable'); "
                "return await bridge.getSceneState(); }))"
            )
        else:
            observed = f"await {_locator(condition.locator)}.innerText()"
        return [
            "await expect.poll(",
            f"  async () => {observed},",
            f"  {{ timeout: {condition.timeout_ms}, intervals: [{condition.interval_ms}] }},",
            f").toContain({json.dumps(condition.expected, ensure_ascii=False)});",
        ]
...
def _locator(locator: Locator | None) -> str:
    ...
    if locator.within:
        root = _locator(locator.within)
    else:
        root = "page"
    for host in locator.shadow_hosts:
        root += f".locator({json.dumps(host, ensure_ascii=False)})"
    if locator.role:
        options = []
        if locator.name:
            options.append(f"name: {json.dumps(locator.name, ensure_ascii=False)}")
        options.append(f"exact: {str(locator.exact).lower()}")
        return f"{root}.getByRole({json.dumps(locator.role)}, {{ {', '.join(options)} }})"
    if locator.label:
        return f"{root}.getByLabel({json.dumps(locator.label, ensure_ascii=False)}, {{ exact: {str(locator.exact).lower()} }})"
    if locator.placeholder:
        return f"{root}.getByPlaceholder({json.dumps(locator.placeholder, ensure_ascii=False)}, {{ exact: {str(locator.exact).lower()} }})"
    if locator.attribute_name:
        return f"{root}.locator({json.dumps('[name=' + json.dumps(locator.attribute_name) + ']', ensure_ascii=False)})"
    if locator.test_id:
        return f"{root}.getByTestId({json.dumps(locator.test_id, ensure_ascii=False)})"
    if locator.href:
        return f"{root}.locator({json.dumps('a[href=' + json.dumps(locator.href) + ']', ensure_ascii=False)})"
    if locator.text:
        return f"{root}.getByText({json.dumps(locator.text, ensure_ascii=False)}, {{ exact: {str(locator.exact).lower()} }})"
    return f"{root}.locator({json.dumps(locator.css or '', ensure_ascii=False)})"
