...
def test_container_mounts_controlled_test_data_read_only(tmp_path: Path) -> None:
    test_data = tmp_path / "test-data"
    test_data.mkdir()
    config = RunnerConfig(artifacts_root=tmp_path, test_data_root=test_data)

    spec = build_container_spec(_plan(), config)
    command = build_docker_command(
        "docker", "ai-gui-runner:1.30.20", tmp_path / "run-3", "run-3", config
    )
    rendered = " ".join(str(item) for item in command)

    assert spec["config"]["test_data_root"] == "/work/test-data"
    assert f"src={test_data.resolve()}" in rendered
    assert "dst=/work/test-data,readonly" in rendered
