...
def test_health_declares_real_engine() -> None:
    ...
    assert response.json()["version"] == "1.30.20"
...
def test_fixed_run_uses_persisted_scenario_and_environment(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(server, "PROJECT_STORE", ProjectStore(tmp_path / "projects"))
    test_data_root = tmp_path / "test-data"
    test_data_root.mkdir()
    monkeypatch.setenv("GUI_TEST_DATA_ROOT", str(test_data_root))
    project = client.post(
        "/api/projects",
    ...
    class FakeOrchestrator:
        def start(self, saved_plan, config):
            ...
            assert config.test_data_root == test_data_root.resolve()
...
def test_replay_uses_configured_test_data_root(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(server, "ARTIFACTS_ROOT", tmp_path)
    run_dir = _write_reviewable_run(tmp_path, "replay-test-data")
    run = json.loads((run_dir / "run.json").read_text(encoding="utf-8"))
    run["generated_test"] = {"stability_level": "A", "supported_replay_modes": ["stable"]}
    (run_dir / "run.json").write_text(json.dumps(run, ensure_ascii=False), encoding="utf-8")
    test_data_root = tmp_path / "test-data"
    test_data_root.mkdir()
    monkeypatch.setenv("GUI_TEST_DATA_ROOT", str(test_data_root))
    ...
    class FakeOrchestrator:
        def run_blocking(self, saved_plan, config):
            assert config.replay_mode == "stable"
            assert config.test_data_root == test_data_root.resolve()
            now = "2026-07-20T00:00:00+08:00"
            return {
                "run_id": "replayed", "plan_name": saved_plan.name, "role": None,
                "base_url_summary": saved_plan.base_url, "status": "passed",
                "started_at": now, "ended_at": now, "steps": [], "assertions": [],
                "reproduction_steps": [], "cause_hints": [], "findings": [],
    ...
    monkeypatch.setattr(server, "RUN_ORCHESTRATOR", FakeOrchestrator())
    response = client.post("/api/runs/replay-test-data/replay", json={"mode": "stable", "headless": True})

    assert response.status_code == 200, response.text
    assert response.json()["run_id"] == "replayed"
