...
interface BackendStep {
  ...
  file_evidence?: Array<{ path: string; sizeBytes: number; sha256: string; mimeType: string; archiveEntries?: string[] }>;
  async_timeline?: Array<{ observed: string; elapsedMs: number }>;
}
...
function toRun(payload: BackendRun): TestRun {
  ...
  const steps = payload.steps.map((step) => ({
    ...
    asyncTimeline: step.async_timeline || []
  }));
  return {
    id: payload.run_id,
