...
function toRun(payload: BackendRun): TestRun {
  ...
  const displayStatus: RunStatus = reviewSummary.disposition === 'pending_confirmation'
...
export const api = {
  ...
  deleteRuns: (runIds: string[]) => request<{ deleted: string[]; count: number }>('/api/runs/delete', {
  ...
  async replay(runId: string, mode: 'stable' | 'adaptive'): Promise<TestRun> {
    return toRun(await request<BackendRun>(`/api/runs/${encodeURIComponent(runId)}/replay`, {
      method: 'POST', body: JSON.stringify({ mode, headless: true })
    }));
  }
};
