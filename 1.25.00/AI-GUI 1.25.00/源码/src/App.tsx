...
import { Activity, ClipboardCheck, Copy, Download, Eye, EyeOff, FileText, History, KeyRound, Play, Save, ScanSearch, ShieldAlert, ShieldCheck, SquarePen, StopCircle, Trash2 } from 'lucide-react';
import { api } from './services/api';
import type { AISettings, ActionType, CompatibilityReport, EnvironmentConfig, EnvironmentDraft, Finding, GeneratedTest, Locator, PlanAssertion, PlanStep, ProjectConfig, ProjectDraft, Report, ReviewedStep, RunPathReview, ScenarioConfig, ScenarioDraft, SessionMetadata, TestCaseDraft, TestPlan, TestRun } from './services/types';

type Page = 'overview' | 'projects' | 'ai' | 'new' | 'run' | 'history' | 'report';
...
const initialProject: ProjectDraft = {
  name: '', baseUrl: '', allowedHosts: [], forbiddenActions: ['删除数据', '支付', '提交生产表单'], onboardingLevel: 'L0',
...
export default function App() {
  ...
  const generatePlan = () => execute(async () => {
    ...
    if (!draft.name.trim() || !draft.preconditions.trim() || !draft.flow.trim() || !draft.expectation.trim()) {
      throw new Error('请补齐测试名称、前置条件、测试目标和预期结果后再生成计划。');
    }
    ...
    const generated = plannerMode === 'ai'
      ? await api.generateAIPlan(draft, aiSettings, selectedProject?.id, selectedEnvironment?.id)
      : await api.generatePlan(draft, selectedProject?.id, selectedEnvironment?.id);
  ...
  const replayRun = (mode: 'stable' | 'adaptive') => execute(async () => {
    if (!report) return;
    const nextRun = await api.replay(report.run.id, mode);
    setRun(nextRun);
    setHistory(await api.getHistory());
    setPage('run');
  ...
  const saveProject = () => execute(async () => {
    ...
    const project = selectedProject
      ? await api.updateProject(selectedProject.id, projectDraft)
      : await api.createProject(projectDraft);
  ...
  const chooseProject = (project: ProjectConfig) => execute(async () => {
    ...
    setProjectDraft({
      ...
      forbiddenActions: project.forbiddenActions, onboardingLevel: project.onboardingLevel, limits: project.limits
    });
    setCompatibility(null);
    setSession(null);
    setSelectedScenario(null);
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {page === 'new' && (
          <section className="content">
            <div className="panel scenario-workbench">
              <div className="panel-title">
            ...
            <div className="form-grid">
              <label>测试名称<input value={draft.name} onChange={(event) => updateScenarioBackedDraft({ ...draft, name: event.target.value })} /></label>
              ...
              <label>前置条件<textarea value={draft.preconditions} onChange={(event) => updateScenarioBackedDraft({ ...draft, preconditions: event.target.value })} /></label>
              <label className="wide">自然语言业务流程<textarea value={draft.flow} onChange={(event) => updateScenarioBackedDraft({ ...draft, flow: event.target.value })} /></label>
...
function pageTitle(page: Page) {
  return { overview: '总览页', projects: '项目接入', ai: 'AI 模型设置', new: '新建测试', run: '执行详情', history: '历史报告', report: '报告详情' }[page];
}
...
const actionLabels: Record<ActionType, string> = {
  ...
  clear: '清空', check: '勾选', uncheck: '取消勾选', hover: '悬停', scroll: '滚动', back: '后退', reload: '刷新', press: '按键', visual_click: '视觉点击', bridge_click: 'Bridge 点击'
};
        ...
        <article className="step-card" key={step.id}>
          <div className="step-copy">
            <div className="step-heading">
              <b>#{step.order}</b>
              <div><strong>{step.action}</strong><span>{step.target}</span><span className="step-classification">{step.executionMode} · 稳定性 {step.stabilityLevel} · {step.stabilityReason}</span></div>
            </div>
