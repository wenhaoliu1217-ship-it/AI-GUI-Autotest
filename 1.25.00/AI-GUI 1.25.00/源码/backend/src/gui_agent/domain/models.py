...
class Step(BaseModel):
    ...
    def _validate_action_shape(self) -> "Step":
        ...
        elif self.action == ActionType.VISUAL_CLICK:
            if self.execution_mode != ExecutionMode.VISUAL or not self.locator or not self.relative_position or not self.visual_target:
                raise ValueError("visual_click 需要 visual 模式、Canvas locator、语义目标和相对坐标")
            if self.stability_level not in {StabilityLevel.B, StabilityLevel.C}:
                raise ValueError("visual_click 稳定性只能为 B 或 C")
